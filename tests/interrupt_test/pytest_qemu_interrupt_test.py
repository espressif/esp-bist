...
def test_interrupt_success(qemu_instance, target):
    ...
    for expected_output in expected_outputs:
        assert expected_output in found_outputs, (
            f"Expected output '{expected_output}' not found in QEMU output"
        )


def interrupt_error_test(
    qemu_debug_instance, gdb_instance, test_name, test_bp, injected_value
):
    """
...
def test_interrupt_source_map_error(qemu_debug_instance, gdb_instance, target):
    interrupt_error_test(
        qemu_debug_instance,
        gdb_instance,
        "test_BIST_Interrupt_Source_Map",
        "bist_interrupt_source_map_count",
        "0",
    )
