...
esp_err_t adc_oneshot_new_unit(const adc_oneshot_unit_init_cfg_t *init_config, adc_oneshot_unit_handle_t *ret_unit)
{
    ...
    unit = calloc(1, sizeof(adc_oneshot_unit_ctx_t));
    ...
    ESP_GOTO_ON_ERROR(esp_clk_tree_src_get_freq_hz((soc_module_clk_t)clk_src, ESP_CLK_TREE_SRC_FREQ_PRECISION_CACHED, &clk_src_freq_hz), err, TAG, "clock source not supported");
...
static bool s_adc_unit_claim(adc_unit_t unit)
{
    s_adc_unit_claimed[unit] = true;
    return true;
}

static bool s_adc_unit_free(adc_unit_t unit)
{
    s_adc_unit_claimed[unit] = false;
    return true;
}
